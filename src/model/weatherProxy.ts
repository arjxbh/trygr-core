...
const HOURLY_PARAMS = '&hourly=temperature_2m,temperature_120m&currentweather=true';
...
export class WeatherProxy {
    ...
    getDetails = async (latitude: string, longitude: string) => {
        const url = `${WEATHER_BASE_URL}?latitude=${latitude.substring(0, 5)}&longitude=${longitude.substring(0, 5)}&${HOURLY_PARAMS}&temperature_unit=${this.temperatureUnit}&windspeed_unit=${this.windspeedUnit}&percipitation_unit=${this.precipitationUnit}&timezone=${this.timezone}`;
